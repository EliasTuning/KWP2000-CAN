...
class J2534:
    ...
    def PassThruIoctl(self, Handle, IoctlID, ioctlInput=None, ioctlOutput=None):
        ...
        if ioctlInput is None:
        ...
        elif hasattr(ioctlInput, 'Parameter') and hasattr(ioctlInput, 'Value'):
            # SCONFIG-like object
            inputParam = SCONFIG()
            ...
            # Simple value (enum or int) - pass as pointer to c_ulong
            # This is used for flags like TX_IOCTL_SET_DLL_DEBUG_FLAGS
            value = ioctlInput.value if hasattr(ioctlInput, 'value') else int(ioctlInput)
            pInput = pointer(c_ulong(value))
        ...
        if ioctlInput:
            if hasattr(ioctlInput, 'Parameter') and hasattr(ioctlInput, 'Value'):
                self.logger.debug(
                ...
                value = ioctlInput.value if hasattr(ioctlInput, 'value') else int(ioctlInput)
                self.logger.debug("    pinput value: " + str(value))
